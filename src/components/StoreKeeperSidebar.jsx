...
export function StoreKeeperTopHeader() {
  ...
  // Lắng nghe event cập nhật avatar từ trang Hồ sơ thủ kho
  useEffect(() => {
    ...
    window.addEventListener("storekeeper-avatar-updated", handleAvatarUpdated);
    return () => {
      window.removeEventListener("storekeeper-avatar-updated", handleAvatarUpdated);
    };
  }, []);

  return (
    <header className="h-14 bg-white border-b border-red-100 flex items-center justify-between px-4 pr-6 text-red-600">
