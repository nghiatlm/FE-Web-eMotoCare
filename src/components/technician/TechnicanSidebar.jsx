...
const sections = [
  {
    label: "Tổng quan",
    items: [{ title: "Dashboard", icon: LayoutDashboard, url: "/technician/dashboard" }],
  },
  {
    label: "Công việc",
    items: [{ title: "Danh sách xe", icon: CalendarCheck, url: "/technician/vehicles" }],
  },
];
          ...
          <div
            ...
            {!isCollapsed && (
              <div>
                ...
                <p className='text-sm text-red-100'>Kỹ thuật viên</p>
              </div>
            )}
          </div>
        </SidebarHeader>
...
export function TechnicianTopHeader() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [displayName, setDisplayName] = useState("Kỹ thuật viên");
  ...
  // Lắng nghe event cập nhật avatar từ trang Hồ sơ kỹ thuật viên
  useEffect(() => {
    ...
    window.addEventListener("technician-avatar-updated", handleAvatarUpdated);
    return () => {
      window.removeEventListener("technician-avatar-updated", handleAvatarUpdated);
    };
  }, []);
