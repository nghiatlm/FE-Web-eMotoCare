...
export default function BatteryDetailPage() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        if (
          parsedData &&
          (parsedData.id || parsedData.sampleCount !== undefined)
        ) {
  ...
  const getStatusColor = (conclusion) => {
    ...
    if (typeof conclusion === "object") {
      if (conclusion.safety && conclusion.safety.includes("quá nhiệt"))
        return "error";
      if (conclusion.safety && conclusion.safety.includes("cảnh báo"))
        return "warning";
      return "success";
    }
  ...
  const currentArray =
    power && power.length > 0
      ? power
      : currentFromData && currentFromData.length > 0
      ? currentFromData
      : [];
                  ...
                  <table className='w-full text-sm'>
                    ...
                    <tbody>
                      ...
                      <tr className='border-b'>
                        ...
                        <td className='text-right p-2 text-red-600'>
                          {socStats.min.toFixed(0)}
                        </td>
                        <td className='text-right p-2 font-semibold'>
                          {socStats.avg.toFixed(1)}
                        </td>
                        <td className='text-right p-2 text-green-600'>
                          {socStats.max.toFixed(0)}
                        </td>
                      </tr>
                      <tr>
                        <td className='p-2 font-medium'>SOH (%)</td>
                        <td className='text-right p-2 text-red-600'>
                          {sohStats.min.toFixed(0)}
                        </td>
                        <td className='text-right p-2 font-semibold'>
                          {sohStats.avg.toFixed(1)}
                        </td>
                        <td className='text-right p-2 text-green-600'>
                          {sohStats.max.toFixed(0)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div className='space-y-6'>
              {(() => {
                ...
                if (typeof conclusion === "string") {
                  ...
                  conclusionObj = conclusion.reduce(
                    (acc, item) => ({ ...acc, ...item }),
                    {}
                  );
                }
                        ...
                        {sections.slice(3).length > 0 && (
                          <Row
                            ...
                            {sections.slice(3).map((section) => {
                              ...
                              return (
                                <Col xs={24} sm={24} md={12} key={section.key}>
                                  <Card
                                    size='small'
                                    style={{
                                      backgroundColor: section.bgColor,
